...
import demandData from '@salesforce/apex/CallCustomerController.demandData';
import { ShowToastEvent } from 'lightning/platformShowToastEvent';

export default class CallCustomerOnDemand extends LightningElement {
    @api recordId;
    @track demandRec;
    ...
    callmobile1() {
        ...
        if (!account) {
            ...
            this.showToast('Error', 'Account data not available');
            return;
        }
        ...
        if (account.RW_Country__c === 'India') {
            ...
            if (account.Mobile_No__c?.length !== 10) {
                this.mobile1error = true;
                this.showToast('Error', 'For India, Mobile 1 should be 10 digits');
            }
        }
        ...
        if (!this.mobile1error) {
            makeCalls({
                ...
                .catch(error => {
                    this.showToast('Error', 'Error occurred while calling: ' + error.body?.message || error.message);
                });
        }
    }
    ...
    callmobile2() {
        ...
        if (!account || !this.secondaryMobileNum) {
            return;
        }
        ...
        if (account.Dialing_Country_2__c === 'India') {
            ...
            if (this.secondaryMobileNum?.length !== 10) {
                this.mobile2error = true;
                this.showToast('Error', 'For India, Mobile 2 should be 10 digits');
            }
        }
