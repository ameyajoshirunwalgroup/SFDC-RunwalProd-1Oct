...
export default class GreetingCallChecklist extends LightningElement {
    ...
    @track reasonValue = '';
    @track currentStatus = '';
    @track otherReasonValue = '';
    @track existingStatus = ''; 
    @track isLocked = false;
    @track isButtonsDisabledConfirmAndSave = false;
    ...
    async loadData() {
        ...
        try {
            ...
            if (result.greetingCallData) {
                this.existingStatus = result.greetingCallData[0].Status__c;
                
                // Check if status is Rejected or Hold
                if (this.existingStatus === 'Rejected' || this.existingStatus === 'Hold' || this.existingStatus === 'Accepted' || this.existingStatus === 'Greetings call Cancelled As Welcome Call Is Generated' || this.existingStatus === 'Block Now Release') {
                    this.isLocked = true;
    ...
    get isOtherReasonRequired() {
        return this.reasonValue === 'OTHERS'; // Ensure 'Others' matches your Picklist API Name exactly
    }
    handleOtherReasonChange(e) {
        this.otherReasonValue = e.target.value;
    }
    handleValueChange(e) {
    ...
    handleFinalSubmit() {
        ...
        if (this.isOtherReasonRequired && (!this.otherReasonValue || this.otherReasonValue.trim() === '')) {
            this.showToast('Required', 'Please specify the other reason', 'error');
            return;
        }
    ...
    submit(status) {
        ...
        if(status == 'Accepted'){
            const markedNo = this.checklistData.filter(row => row.isVisible &&
                row.status === 'No' && row.api != 'FundingType'
            );
