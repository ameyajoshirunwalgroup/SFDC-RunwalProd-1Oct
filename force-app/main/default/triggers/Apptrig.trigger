trigger Apptrig on Applicant_Details__c (After insert, After update) 
{
    ...
    if(!byPassTriggerExceution)
    {
        ...
        if(Trigger.isUpdate)
        {
            List<LockatedApp_UserDetailUpdate.ApplicantUpdateWrap> appUpdateWrapperList = new List<LockatedApp_UserDetailUpdate.ApplicantUpdateWrap>(); //Added by Vinay 28-02-2026
            List<Id> addressChangedApplicantIds = new List<Id>(); //Added by Vinay 28-02-2026
            List<Id> phoneChangedApplicantIds = new List<Id>(); //Added by Vinay 28-02-2026
            for(Applicant_Details__c app : trigger.new)
            {
                ...
                //Added by Vinay 28-02-2026 Start
                ...
                if((app.Permanent_Address__c != null && app.Permanent_Address__c != Trigger.oldMap.get(app.Id).Permanent_Address__c) ||
                  ...
                  (app.Permanent_Address_Line_3__c != null && app.Permanent_Address_Line_3__c != Trigger.oldMap.get(app.Id).Permanent_Address_Line_3__c)){  //Added by Vinay 28-02-2026
                    addressChangedApplicantIds.add(app.Id);
                }
                if(app.Mobile_Number__c != null && app.Mobile_Number__c != Trigger.oldMap.get(app.Id).Mobile_Number__c){ //Added by Vinay 28-02-2026
                    phoneChangedApplicantIds.add(app.Id);
                }
            }
            if(appUpdateWrapperList.size() > 0){ //Added by Vinay 28-02-2026
                system.enqueuejob(new LockatedApp_UserDetailUpdate(appUpdateWrapperList));
            }
            if(addressChangedApplicantIds.size() > 0){ //Added by Vinay 28-02-2026
               LockatedApp_Notifications.correspondenceAddressChangeNotification(addressChangedApplicantIds);
            }
            if(phoneChangedApplicantIds.size() > 0){ //Added by Vinay 28-02-2026
                LockatedApp_Notifications.phoneNumberChangeNotification(phoneChangedApplicantIds); 
            }
        ...
        if(Trigger.isInsert)
        {
            List<Id> appIds = new List<Id>(); //Added by Vinay 28-02-2026
            for(Applicant_Details__c app : trigger.new)
            {
                system.debug('*inside for 1*');
                system.debug(app.id);
                ...
                if(app.Mobile_Number__c != null && app.Email_Address__c != null){ //Added by Vinay 28-02-2026
                    appIds.add(app.Id);
                }
            }
            Map<Id,Id> BookingOppMap = new Map<Id,Id>();
            ...
            if(appIds.size() > 0){ //Added by Vinay 28-02-2026
